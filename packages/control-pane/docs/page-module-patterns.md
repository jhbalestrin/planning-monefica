# Control Pane — page module pattern
...
Use React Router `Link` with MUI `component={Link}` for in-app navigation where appropriate (see `.cursor/rules/control-pane-button-navigation.mdc`).
